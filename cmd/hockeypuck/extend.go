...
func (ec *extendCmd) Name() string { return "extend" }

func (ec *extendCmd) Desc() string { return "Extend OpenPGP keyring files into database" }

func newExtendCmd() *extendCmd {
	cmd := new(extendCmd)
...
func (ec *extendCmd) insertPtreeKeys(ptree recon.PrefixTree, inStat <-chan *loadStatus) (chan *loadStatus, chan interface{}) {
...
func (ec *extendCmd) insertDbKeys(db *openpgp.DB, inStat <-chan *loadStatus) (done chan interface{}) {
	done = make(chan interface{})
	go func() {
		defer close(done)
		var err error
...
func (ec *extendCmd) readAllKeys(path string) chan *loadStatus {
	c := make(chan *loadStatus)
